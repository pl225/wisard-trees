...
def generate_distances_network_part3():
    ...
    while(isPickle('graphs-layer-'+str(layer))):
        ...
        for v,neighbors in graphs.items():
            ...
            e_list = [x / sum_w for x in e_list] if sum_w > 0 else [0] * len(e_list)
            weights[v] = e_list
...
def generate_distances_network(workers):
    ...
    t0 = time()
    os.system("rm "+"pickles/distances_nets_weights-layer-*.pickle")
    os.system("rm "+"pickles/alias_method_j-layer-*.pickle")
    os.system("rm "+"pickles/alias_method_q-layer-*.pickle")
    with ProcessPoolExecutor(max_workers=1) as executor:
